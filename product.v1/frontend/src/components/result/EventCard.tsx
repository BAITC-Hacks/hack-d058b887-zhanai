          ...
          <div className="mt-2 flex flex-wrap gap-1 pl-6">
            {event.effects.map((e, i) => (
              <Chip key={i} tone="critical">
                {catalog.indicators.find((x) => x.id === e.indicator)?.name} {signed(e.delta, 1)} {catalog.districts.find((d) => d.id === e.districtId)?.locative}
              </Chip>
            ))}
            {event.budgetDelta !== 0 && <Chip tone="critical">бюджет {signed(event.budgetDelta, 1).replace(',0', '')} → {event.budget}</Chip>}
          </div>
