...
Оформление повторяет `Astana Scheme`: фон `#F3F5F4`, районная палитра, белые подписи-капсулы и шрифты Onest/Unbounded. Шрифты включены в сборку; карта использует локальные контуры и работает без сетевых картографических сервисов.
...
7. **Событие, команды, презентация.** «Неожиданное событие → Разыграть» меняет показатели или бюджет. Например, «Секвестр бюджета» снижает лимит до 90, и план за 95 становится недопустимым, пока вы его не пересоберёте. «Команды» записывают Score в общий лидерборд. «Скачать презентацию» сохраняет `akim-plan.md`.
...
flowchart LR
  ...
  API --> O["optimizer.py<br/>полный перебор 694 395 планов,<br/>кэш на диске"]
...
- **Оптимизатор** перебирает все допустимые наборы (694 395 для правил датасета) быстрой версией той же формулы. Тест сверяет её с основным симулятором. Результат кэшируется в `backend/data/cache/`, и дальше выдача процентиля, оптимума и лучшей замены занимает доли секунды.
- **События** (4 сценария) меняют исходные показатели или бюджет до расчёта мер; база и лимит пересчитываются.
...
4. Если проверка не пройдена, ключа нет или API недоступен (таймаут 16 с), пользователь получает шаблонное объяснение из тех же фактов со статусом `fallback_no_ai`. В интерфейсе оно помечено «без AI · из фактов расчёта», в презентации — «Шаблонное объяснение без AI». Выдать шаблон за ответ модели нельзя.
...
| `find_improvements` | полный перебор 694 395 допустимых планов: процентиль, оптимум, лучшая замена одной меры или лучшее дополнение неполного плана |

Цикл: агент проверяет текущий план → выдвигает 1–4 гипотезы (с учётом цели пользователя, например «подтянуть Нуру и уложиться в 90») → проверяет каждую инструментом → возвращает вывод, риски и один рекомендуемый план. Сервер **заново симулирует** рекомендованный план (агенту на слово не верим), а каждое число в тексте агента сверяет с результатами вызовов инструментов; если число не подтверждено, в интерфейсе появляется пометка. Весь ход работы агента (какие планы он проверил и что получил) показывается пошагово, рекомендованный план применяется одной кнопкой.

Включение: впишите `OPENAI_API_KEY` в `product.v1/.env` и перезапустите `./scripts/start.sh`. Без ключа агент честно отвечает `unavailable`, остальной продукт работает. Лимит вызовов инструментов — `AGENT_MAX_TOOL_CALLS` (по умолчанию 8). Цикл агента покрыт тестом со сценарной моделью (`test_agent_tool_loop_with_scripted_model`): проверяются вызовы инструментов, пересчёт рекомендации и срабатывание проверки чисел.
...
| *Опц.* AI-рекомендации по улучшению | `recommendations` в `/api/explain` + `POST /api/improve` (полный перебор: процентиль, оптимум, лучшая замена); UI: «Улучшить план» → «Применить» |
| *Опц.* Неожиданные события с перераспределением бюджета | `events.py`, `POST /api/events/draw`, `eventId` в `/api/simulate`; UI: «Неожиданное событие» → «Разыграть / Другое событие / Сбросить» |
...
| Проверка 5: изменение решений меняет Score | пример 56.54 → замена M5 на M3 в Нуре 57.21 → оптимум 57.24; тесты `test_improve_and_completion`, `test_fast_search_score_matches_public_simulator_across_plans` |
...
curl -s -X POST http://127.0.0.1:8000/api/explain -H 'Content-Type: application/json' -d "$PLAN" \
  | python3 -c 'import json,sys; r=json.load(sys.stdin); print(r["status"], r["grounded"]); print(r["summary"])'
```
...
**Ограничения.** Данные синтетические, из датасета хакатона. Score показывает последствия по заданной модели, а не прогноз для реальной Астаны. Эффекты мер линейные и не зависят друг от друга, кроме трёх синергий. Лидерборд хранится локально на одном сервере. Поиск оптимума после события пока не поддерживается (`/api/improve` возвращает 422).
