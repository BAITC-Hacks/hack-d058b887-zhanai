...
2. **Выбор мер.** В «Каталоге мер» слева нажмите «Выбрать район» (для районной меры) или «Добавить» (для городской). В подсказке выбора района виден текущий уровень нужного показателя, а худший район помечен «нужнее всего». Недоступная мера спокойно объясняет причину: не хватает бюджета, превышен лимит направления или мера несовместима с уже выбранной. На карточке меры видно реальный эффект за горизонт (например, «+4,5» вместо «+6») и полоску «работает 6 из 8 кварталов». Когда собраны все пять решений, вверху каталога появляется плашка «План собран», а у каждой меры — кнопка «Заменить»: выбираете, какую меру из плана она вытеснит, и (для районной меры) район.
...
6. **Улучшение.** Карточка «Улучшить план» предлагает лучшую замену одной меры с новым Score (для примера: M5 в Сарыарке → ЛРТ в Нуре, **57.21**) и глобальный оптимум **57.24**. Кнопки «Применить» и «Показать» меняют план, и Score сразу пересчитывается. Над Score шкала «база → план → оптимум» показывает, сколько осталось до оптимума. Лучшая замена также подсвечивается прямо на карточке меры в каталоге и в блоке «Что сделать» AI-анализа. Для недопустимого плана Score не показывается: вместо числа — «—» и список нарушений с подсказкой, что поменять.
