...
interface Props {
  ...
  showAfter: boolean
  /** План ещё не допустим: оценки «после» — предварительный расчёт. */
  provisional?: boolean
...
// Подсказку «нажмите на район» показываем, пока пользователь ни разу не выбрал район.
const HINT_KEY = 'akim.mapHintSeen.v1'
...
function writeHintSeen() {
  try {
    ...
    /* хранилище недоступно — подсказка просто скроется до перезагрузки */
  }
}

export function CityMap({ catalog, districts, showAfter, provisional = false, selected, onSelect }: Props) {
  const [hintSeen, setHintSeen] = useState(readHintSeen)
  // Район открыли — с картой или из портфеля: подсказка больше не нужна.
  useEffect(() => {
    if (selected === null || hintSeen) return
    setHintSeen(true)
    writeHintSeen()
  }, [selected, hintSeen])
  const showHint = !hintSeen && selected === null
  ...
  return (
    <section aria-label="Карта районов" className="card p-4">
      <SectionTitle aside={selected ? 'нажмите ещё раз, чтобы снять выбор' : provisional ? 'предварительно' : undefined}>Город</SectionTitle>

      {showHint && (
        <p className="mb-2 flex justify-center">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-accent-soft px-3 py-1 text-[12.5px] leading-snug font-medium text-accent-ink animate-fade-up">
            <MousePointerClick aria-hidden="true" className="size-3.5 shrink-0" />
            Нажмите на район, чтобы увидеть 10 показателей
          </span>
        </p>
      )}
      ...
      <p className="mt-3 text-center text-[11px] text-ink-3">
        {showAfter
          ? provisional
            ? 'на подписи: оценка района до → после (предварительно: план ещё не допустим)'
            : 'на подписи: оценка района до → после плана'
          : 'на подписи: оценка района сейчас'}
        {!showHint && ' · нажмите район, чтобы увидеть 10 показателей'} · границы: © OpenStreetMap
      </p>
    </section>
  )
}
