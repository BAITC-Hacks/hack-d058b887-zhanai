...
type Action =
  ...
  | { type: 'swap'; index: number; decision: Decision }
  | { type: 'clear' }
...
function reducer(state: PlanState, action: Action): PlanState {
  switch (action.type) {
    ...
    case 'swap':
      if (state.decisions.some((d, i) => i !== action.index && d.measureId === action.decision.measureId)) return state
      return {
        decisions: state.decisions.map((d, i) => (i === action.index ? { ...action.decision } : d)),
        version: state.version + 1,
      }
    case 'replace':
...
export function usePlan() {
  ...
  /** Заменить решение с индексом `index` на новое (меру из каталога). */
  const swap = useCallback((index: number, decision: Decision) => dispatch({ type: 'swap', index, decision }), [])
  const clear = useCallback(() => dispatch({ type: 'clear' }), [])
  return { decisions: state.decisions, version: state.version, add, remove, setDistrict, replace, swap, clear }
}
