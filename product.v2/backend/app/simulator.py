...
def _metrics(indicators: dict[str, dict[str, float]]) -> dict:
    ...
    critical = sum(value < CRITICAL_THRESHOLD for row in indicators.values() for value in row.values())
    weakest = min(scores, key=scores.get)
    return {"score": 0.7 * average + 0.3 * minimum - critical, "cityAverage": average, "minDistrictScore": minimum, "minDistrictId": weakest, "criticalCount": critical, "districtScores": scores}


CRITICAL_THRESHOLD = 40


def critical_ids(row: dict[str, float]) -> list[str]:
    """Indicator IDs below the critical threshold, in the dataset's indicator order."""
    return [key for key in RULES["indicatorWeights"] if row[key] < CRITICAL_THRESHOLD]
...
def simulate(decisions: list[Decision], base_indicators: dict[str, dict[str, float]] | None = None, budget: int | None = None) -> dict:
    ...
    districts = [{"districtId": d["id"], "name": d["name"], "beforeScore": base_metrics["districtScores"][d["id"]], "afterScore": metrics["districtScores"][d["id"]], "beforeIndicators": base[d["id"]], "afterIndicators": indicators[d["id"]], "criticalBefore": critical_ids(base[d["id"]]), "criticalAfter": critical_ids(indicators[d["id"]])} for d in DISTRICTS]
    ...
    return {"score": metrics["score"], "delta": metrics["score"] - base_metrics["score"], "cityAverage": metrics["cityAverage"], "minDistrictScore": metrics["minDistrictScore"], "minDistrictId": metrics["minDistrictId"], "criticalCount": metrics["criticalCount"], "districts": districts, "synergies": applied, "decisions": resolved, "directionCoverage": coverage, "missedSynergies": missed, "unusedBudget": active_budget - cost}
